...
val MIGRATION_1_2 = object : Migration(1, 2) {
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        val cursor = database.query("SELECT DISTINCT theme_name FROM inspirations WHERE content != '__THEME_MARKER__'")
        ...
        database.execSQL("DROP TABLE inspirations")
        database.execSQL("ALTER TABLE inspirations_new RENAME TO inspirations")
        
        // Create indices on the new table
        database.execSQL("CREATE INDEX index_inspirations_content ON inspirations(content)")
        database.execSQL("CREATE INDEX index_inspirations_theme_name ON inspirations(theme_name)")
    }
}